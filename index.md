---
layout: default
title: Kreator CAD
permalink: /
---

<div class="hero" style="text-align: center; padding: 50px 0;">
  <h1>Kreator CAD</h1>
  <p class="lead" style="font-size: 1.2em; color: #57606a;">
    Система Автоматизированного Проектирования<br>
    для создания Систем Организационного Управления (СОУ)
  </p>
  
  <br>
  
  <div style="margin: 20px 0;">
    <a href="./kreator_setup.zip" class="btn-download" download>
      🛠 Скачать Среду Проектирования
    </a>
  </div>
  
  <p style="font-size: 0.9em; color: #666;">
    Версия: v0.1 (Alpha) • Лицензия: Private
  </p>
</div>

---

## ⚡️ Что такое Kreator?

**Kreator** — это инструмент для быстрой разработки строгих управленческих систем. Он позволяет описать логику организации (кто, что и как делает) и автоматически сгенерировать готовое рабочее место для сотрудников.

* **Фокус на логике:** Вы описываете смыслы, а не рисуете кнопки.
* **Pain-Driven Development:** Инструментарий выдает только то, что необходимо для снятия организационной боли.
* **Две коробочки:** Чёткое разделение среды разработки и среды исполнения.

> *"Мы не продаём софт. Мы передаём зафиксированный в инструменте способ деятельности."*

---

## 🚀 Установка САПР

Процесс идентичен установке любой системы на платформе Kreator.

### Шаг 1. Платформа
Убедитесь, что у вас установлен **VS Code**.
[Скачать с code.visualstudio.com](https://code.visualstudio.com/)

### Шаг 2. Распаковка
1. Скачайте архив **`kreator_setup.zip`** по кнопке выше.
2. Распакуйте его в удобное место (например, `C:\Projects\Kreator`).

### Шаг 3. Запуск
В распакованной папке запустите скрипт установки:
* **Windows:** `install_windows.bat`
* **Mac/Linux:** `install_mac.sh`

---

## 🏁 Начало работы

После запуска откроется среда проектирования. 
Чтобы создать вашу первую систему:

1. Откройте файл `README.md` внутри редактора (он откроется автоматически).
2. Следуйте инструкции "Быстрый старт" для генерации пробного проекта.

---

<style>
/* Тот же стиль кнопки, что и на страницах ИС, для консистентности */
.btn-download {
  display: inline-block;
  background-color: #0969da; /* Синий цвет для "Инженерного" инструмента */
  color: white;
  padding: 14px 28px;
  text-decoration: none;
  border-radius: 6px;
  font-weight: bold;
  font-size: 18px;
  box-shadow: 0 3px 5px rgba(0,0,0,0.1);
  transition: all 0.2s;
}

.btn-download:hover {
  background-color: #0860ca;
  transform: translateY(-1px);
  color: white;
  text-decoration: none;
}

.hero {
  background-color: #f6f8fa;
  border-bottom: 1px solid #d0d7de;
  margin-bottom: 30px;
  border-radius: 6px;
}
</style>